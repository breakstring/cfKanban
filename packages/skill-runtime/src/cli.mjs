...
const COMMANDS = new Map([
  ...
  ["runtime resolve-cloudflare-auth", command({ description: "Resolve the effective Wrangler authentication context first; only list or inspect another profile when that context is unusable.", effect: "read_only_local_auth_and_cloudflare_accounts", inputFields: ["wranglerExecutable", "contextDirectory", "selectedProfile"], surfaces: ["deploy"], run: resolveCloudflareAuth })],
