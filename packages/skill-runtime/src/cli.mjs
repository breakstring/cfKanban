...
const COMMANDS = new Map([
  ...
  ["release discover", command({ description: "Discover the latest canonical stable release, or one explicitly selected exact version, without installing or downloading bundles. Pin the returned manifest before planning; artifact verification remains required.", effect: "credential_free_read_only_network", inputFields: ["version"], surfaces: ["deploy"], run: discoverRelease })],
  ...
  ["plan instance-upgrade", command({ description: "Create a complete existing-resource Cloudflare Instance upgrade plan without changing local Skills.", effect: "plan_only", inputFields: ["taskId", "instanceId", "operationId", "cloudflare", "resources", "bindings", "owner", "current", "target", "migrations", "restorePoint", "allow_breaking_change", "attachments", "usageAnalytics"], surfaces: ["deploy"], run: (input) => { const plan = createInstanceUpgradePlan(input); return { plan, plan_digest: canonicalDigest(plan) }; } })],
