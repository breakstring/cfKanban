...
const COMMANDS = new Map([
  ...
  ["plan instance-upgrade", command({ description: "Create a complete existing-resource Cloudflare Instance upgrade plan without changing local Skills.", effect: "plan_only", inputFields: ["taskId", "instanceId", "operationId", "cloudflare", "resources", "bindings", "owner", "current", "target", "migrations", "restorePoint", "allow_breaking_change", "attachments"], surfaces: ["deploy"], run: (input) => { const plan = createInstanceUpgradePlan(input); return { plan, plan_digest: canonicalDigest(plan) }; } })],
