...
async function seed(db) {
  await db.batch([
    ...
    db.prepare("INSERT INTO projects (id, workspace_id, display_name, issue_limit, comment_limit, principal_limit, created_at, updated_at, created_by_principal_id, updated_by_principal_id, created_operation_id) VALUES ('project', 'workspace', 'Core', 10, 1, 10, ?, ?, 'owner', 'owner', 'seed-project')").bind(now, now),
    db.prepare("INSERT INTO public_join_policies (project_id, workspace_id, public_id, public_summary, enabled_at, enabled_by_principal_id, created_at, updated_at) VALUES ('project', 'workspace', 'public', 'Public', ?, 'owner', ?, ?)").bind(now, now, now),
