...
- v0 同时支持 Windows 原生与 WSL2，但将其视为互不混用的独立执行环境。Agent 只解析当前环境内的 Node、Wrangler、Skills、Tool Runtime、Cloudflare auth 与 cfKanban Credential，不跨 Windows/WSL 边界自动发现、调用、复制或共享。
...
- 首次部署默认零参数生成 strict-zero 候选：单一 Cloudflare account/profile 明确时，Agent 自动解析 stable deployment bundle、提议无冲突资源名并生成 bindings、非秘密 IDs 与 operation metadata，用户只需一次授权完整 plan。计划只声明 Owner Credential 生成/保存策略，secret 在授权后的执行阶段直接落入受限文件。只有 account 歧义，或 custom domain、付费能力、数据地域/合规、非 stable/源码试验等结果性偏差才询问；未知资源不接管，默认另提名称。
