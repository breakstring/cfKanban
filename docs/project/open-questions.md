...
- Q-101 已于 2026-08-28 确认：完成结果保存为结构化、不可变且不可删除的 `kind=completion` Comment，不建立独立 Completion 实体。`summary` 选填（2026-09-20 用户授权修订；省略或纯空白归一为空字符串），`verification`、`artifacts`、`follow_ups` 可为空；complete 在一个原子单元中校验 version、追加记录、转为 `done` 并写 Event。reopen 保留旧记录，再次完成追加新记录，普通状态更新不能绕过 complete 直接转入 `done`。
