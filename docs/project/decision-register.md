...
Foundation 的领域与服务端公共技术选择、Agent-first Storyboard 及首发 Skill/部署体验已经完成第一轮收敛，两份 SPEC 已于 2026-08-28 按 D-212 冻结。Foundation 当前按 D-245 修订到 18；Agent Skills & Bootstrap 按 D-246/D-247 修订到 20，固定可重现构建、migration manifest/checksum/schema readback，并明确部署型 GitHub Actions 后置、v0 保持 Agent-first 唯一主部署路径；此前 monorepo、同 Worker Static Assets、Passkey、preferred origin、安全自动 rebind、Public Join 恢复、quota 隔离和恢复合同继续有效。D-235/D-238/D-248/D-249 进入 Draft Web UI 与根目录 `DESIGN.md` 合同。Web UI 和 API/Schema 仍为 Draft，不因此获得实现授权。
