...
当前测试发行指针是 <https://github.com/breakstring/cfKanban/releases/download/0.1.0-alpha.8/prerelease.json>。只有用户明确选择测试发行版后，`cfkanban-deploy` 才能使用它。
...
提出 Cloudflare 登录前，`cfkanban-deploy` 会先使用 `runtime resolve-cloudflare-auth` 发现可复用的 profile/account 映射；它不会返回 token、邮箱、目录绑定、资源清单或 Wrangler 原始输出。只有一个已验证映射时直接复用，存在多个映射时交给用户选择；既有 profile 无法解析时停止，不会把它误判成“尚未登录”。只有确实没有可用认证时，才继续使用 `runtime inspect-cloudflare-auth`、`runtime plan-cloudflare-auth` 与获批后的 `runtime cloudflare-auth-action`。登录计划会明确所选 Wrangler profile、浏览器或 device 交互、最小必需 OAuth scopes，以及启用 Wrangler keyring 会改变当前操作系统用户所有 Wrangler profiles 的全局偏好。登录本身不会创建 Worker 或 D1 资源。
