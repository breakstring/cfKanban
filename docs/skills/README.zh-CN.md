...
当前测试发行指针是 <https://github.com/breakstring/cfKanban/releases/download/0.1.0-alpha.6/prerelease.json>。只有用户明确选择测试发行版后，`cfkanban-deploy` 才能使用它。
...
Cloudflare 登录时，`cfkanban-deploy` 会先使用 `runtime inspect-cloudflare-auth`，再使用 `runtime plan-cloudflare-auth`；只有准确计划获批后，才会按顺序调用其中的 `runtime cloudflare-auth-action`。计划会明确所选 Wrangler profile、浏览器或 device 交互、最小必需 OAuth scopes，以及启用 Wrangler keyring 会改变当前操作系统用户所有 Wrangler profiles 的全局偏好。登录本身不会创建 Worker 或 D1 资源。
