...
The testing release linked by this guide is `0.1.0-alpha.50`. Disclose that it is a prerelease and require the user's explicit testing choice before selecting it; do not silently substitute it for stable. Reuse an already compatible, verified installation. Installing or updating local Skills is a separate operation from deploying or upgrading an instance.

If running in Codex and installation is needed, first present this host-level plan: source `https://github.com/breakstring/cfKanban.git`, immutable ref `0.1.0-alpha.50`, user scope, and removal of the newly installed plugin as rollback. After authorization, use these commands for a fresh installation:
...
<https://github.com/breakstring/cfKanban/releases/download/0.1.0-alpha.50/prerelease.json>
