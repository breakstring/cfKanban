...
当前测试发行版是 `0.1.0-alpha.34`。它仍是预发行版，因此 Agent 必须明确告诉你这一点，并在选用前取得你对测试版本的明确选择。

在 Codex 中，先让 Agent 展示这项宿主级安装计划：来源 `https://github.com/breakstring/cfKanban.git`、不可变 ref `0.1.0-alpha.34`、用户级安装范围，以及卸载 plugin 的回退方式。全新安装时使用：
...
> 请使用 `$cfkanban-deploy` 和这份指南为我部署一个新的 cfKanban 实例。使用 `0.1.0-alpha.34` 测试发行版。先做只读检查；任何本地安装或 Cloudflare 写入前，先展示准确计划和所需授权。
