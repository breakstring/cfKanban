...
本指南链接的测试发行版是 `0.1.0-alpha.49`。明确说明它是预发行版，选用前确认用户已明确选择测试版本，不得静默替代 stable。已有兼容且经过验证的安装应优先复用；本地 Skill 安装或更新与云端实例部署或升级是不同操作。

如果当前宿主是 Codex 且确需安装，先展示宿主级计划：来源 `https://github.com/breakstring/cfKanban.git`、不可变 ref `0.1.0-alpha.49`、用户级范围，以及卸载本次新装 plugin 的回退方式。获得授权后，全新安装使用：
