...
6. If deployment is requested, start with read-only checks. Reuse a journal/receipt's exact Wrangler target, or let Wrangler resolve the current private deployment/config context; consider only the sole or user-selected alternate profile if that context fails. Pin the chosen `account_id` in the private config, explain the proposed Cloudflare resources in plain language, and wait at the required authorization boundary.
