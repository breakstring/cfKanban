...
The current public testing pointer is `https://github.com/breakstring/cfKanban/releases/download/0.1.0-alpha.12/prerelease.json`. Treat it as unavailable until that exact HTTPS resource and its declared immutable manifest/artifacts can be fetched and verified. Do not substitute the repository tag, plugin cache, or source checkout for a missing release asset.
...
11. Immediately before Worker deployment, require its exact readback to remain `absent`. After a successful deploy, require it to return `present`. Use `deployment prepare-owner-credential` so the private pending Credential is derived from the authorized Owner IDs rather than caller-supplied IDs. `bootstrap write-owner-sql` must recheck the journal, frozen config, complete migration readback, successful Worker deployment, exact origin, and pending Credential before journaling the SQL digest; `action=bootstrap_owner` accepts only that exact private file. After a failed, interrupted, or response-uncertain attempt, run `action=owner_bootstrap_readback`. Its fixed `SELECT` returns only bounded row counts for the six tables touched by bootstrap and disables Wrangler disk logs. A newer `absent` result permits exactly one retry of the same SQL; `present_or_partial`, a successful prior attempt, malformed output, or a missing newer readback forbids retry and requires finalization/readback or a stop.
