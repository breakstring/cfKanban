...
- Resolve the effective Wrangler authentication context first, consider other profiles only when that context is unusable, and never return tokens, email, directory bindings, resource inventories, or raw output; create and execute an exact, task-bound OAuth plan only when login is actually missing; then read back the selected account before freezing a deployment plan.
...
The current public testing pointer is `https://github.com/breakstring/cfKanban/releases/download/0.1.0-alpha.9/prerelease.json`. Treat it as unavailable until that exact HTTPS resource and its declared immutable manifest/artifacts can be fetched and verified. Do not substitute the repository tag, plugin cache, or source checkout for a missing release asset.
...
| Resolve Cloudflare auth | `runtime resolve-cloudflare-auth` | Run `whoami` in the private deployment context first. Only if it is unusable, list profile names; inspect the sole or explicitly selected profile only. Never return tokens, email, bindings, resource inventories, or raw output. |
...
When resuming an authorized journal or maintaining an installed instance, first verify and reuse the exact profile/account already frozen there. Otherwise run `runtime resolve-cloudflare-auth` with the resolved Wrangler executable and the private deployment/config context directory. It follows Wrangler's own priority: environment credentials, then the profile bound to that context or the default profile. A successful current context is used without enumerating other profiles. If the current context is unusable, the resolver may list bounded profile names; it probes account memberships only for the sole profile or the one the user explicitly selects. An unrelated or stale profile is never inspected and cannot block a usable selected context.

The resolver may hold the current or selected Wrangler token only inside the bounded helper process while obtaining account memberships. The token, user email, directory bindings, Cloudflare resource lists, and raw command output never leave the helper or enter receipts. Treat returned account/profile labels only as untrusted display metadata. Handle its states exactly: `resolved` means read back the exact account; `account_selection_required` asks only for an account; `profile_selection_required` asks only for a profile and then reruns the resolver with `selectedProfile`; `unavailable` permits a login proposal; `blocked` stops on the reported active/selected-context problem.
...
4. Reuse an exact profile/account already frozen in the authorized journal/receipt. Otherwise run `runtime resolve-cloudflare-auth` in the private deployment/config context. Use the effective context when it works. Only after it fails may the resolver list profile names; verify only the sole or explicitly selected profile. Ask only for the unresolved profile or account choice. Only `unavailable` permits a new OAuth plan; `blocked` stops. Skill or Service version changes never justify a new login. Do not bind a profile to the Repo.
...
10. Generate the private Wrangler config from the verified bundle. Execute only plan-listed actions, reconcile ledger checksums and schema artifacts before and after migrations, and run the allowlisted Worker dry run. Migration SELECT readback uses `d1 execute --command --json`; remote `--file` uses Wrangler's ingestion path and is not a row-returning readback.
...
- **MUST:** Before proposing login, reuse the journal/receipt's exact verified auth target or run `runtime resolve-cloudflare-auth` in the private deployment context. Prefer the effective context; inspect no unrelated profile; ask only when the profile or account is genuinely ambiguous.
