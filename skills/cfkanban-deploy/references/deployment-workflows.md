...
2. Otherwise run `runtime resolve-cloudflare-auth` with the resolved absolute Wrangler executable and the private deployment/config context directory. It first runs `whoami` in that context, allowing Wrangler's normal precedence to choose environment authentication, a directory-bound profile, or the default profile. It does not inspect any other profile when this current context works.
3. If the current context is unusable, the resolver lists only bounded profile names. One available profile may be checked directly; with several, `profile_selection_required` asks the user to choose a profile and the command is rerun with `selectedProfile`. Only that profile's account memberships are inspected. `account_selection_required` asks only for an account; `resolved` advances to exact readback; `unavailable` permits a new login proposal; `blocked` stops on a problem in the active or selected context. An unrelated or stale profile is never probed and cannot block the selected context.
...
| Existing Cloudflare auth | `runtime resolve-cloudflare-auth` | Effective context first; only names alternate profiles after failure, and only checks the sole or selected profile. No token, email, directory binding, resource inventory, or raw output. |
