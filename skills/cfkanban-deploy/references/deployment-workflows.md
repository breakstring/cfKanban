...
| Owner bootstrap | `deployment prepare-owner-credential`, `bootstrap write-owner-sql`, `deploy wrangler-action` with `bootstrap_owner`, `deployment finalize-owner` | Plan-bound pending secret and hash-only SQL, one journaled D1 attempt, exact discovery/`/meta`/`/me` verification, promotion, and redacted receipt; none reveals the token. |
