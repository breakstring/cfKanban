...
1. First reuse a known profile/account pair when it still passes `runtime wrangler-account-readback`; a Skill or Service release change is not a reason to log in again. Profile names identify authentication contexts and never include cfKanban release/version labels. With no known prior choice, use the stable candidate `cfkanban` for local interactive use, or `default` for a device-flow environment. Run `runtime inspect-cloudflare-auth` with the resolved absolute Wrangler executable and that profile name. It checks the exact Wrangler version, command support, keyring preference, profile existence, required scope catalog, and environment-variable shadowing without returning a token, credential path, profile list, or raw Wrangler output.
...
5. Reuse a known profile/account that still passes readback, regardless of the current Skill or Service version. If Cloudflare auth is not already usable, inspect the stable candidate profile and generate the task-bound OAuth plan. Show the profile operation, four requested scopes, global keyring effect, browser/device interaction, local storage owner, and exact digest. Execute only authorized `runtime cloudflare-auth-action` entries, then inspect again. Never use a versioned profile name, `login --profile`, one quoted argument for all scopes, a Repo binding, or broad product scopes.
