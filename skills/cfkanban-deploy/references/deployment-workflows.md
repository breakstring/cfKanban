...
- `service-releases/` contains verified immutable Service artifacts used by deployment/upgrade plans; it is independent of active Skills.
...
The default strict-zero OAuth plan requests only these four scopes, each as a separate process argument after one `--scopes` flag:
...
Cloudflare automatically adds `offline_access` so Wrangler can refresh the OAuth login. The default plan omits the broad `workers:write` scope and separate KV, routes, Pages, zone, AI, Queue, DNS, and other product scopes. For explicitly requested R2 attachment storage, pass `attachmentStorage: true` to both `runtime inspect-cloudflare-auth` and `runtime plan-cloudflare-auth`. This adds only `workers:write`, because pinned Wrangler 4.127.1 exposes no R2-only OAuth scope. It grants broad Workers data access (including KV, scripts, and routes), not one-bucket access. Cloudflare’s [official bindings MCP](https://github.com/cloudflare/mcp-server-cloudflare/blob/main/apps/workers-bindings/src/bindings.app.ts) uses this legacy OAuth scope for its R2 tools. Existing profiles also require `allowExistingProfile: true` and explicit re-authentication approval. Do not invent `r2:write` or treat a 403/code 10000 as proof that the subscription is absent. After consent, read back the exact account and R2 bucket with `runtime r2-storage-readback`; success of OAuth alone is insufficient.

If Wrangler no longer exposes a required scope, or the Cloudflare consent surface requires an unexpected scope, stop and require a corrected release; do not widen access in place.
...
This workflow restores access after total Owner API Credential loss. It preserves the original Owner Principal, display name, business data, history, and Passkeys; it revokes all previous Owner API Credentials, so their Launch/Session access also becomes invalid. Passkey sessions remain governed by their independent authentication source. A present local current secret must use normal Owner rotation through `cfkanban-admin`. Existing metadata with a missing secret is recoverable. When the target is unknown, discover candidates in the already selected Cloudflare account and ask the user to choose the exact instance; never guess an account/resource or silently trust an origin.

For an unknown target, run `owner-recovery discover` with `accountId`, `wranglerExecutable`, and exactly one of `cloudflareProfile` / `contextDirectory`; optional `workerNames` narrows the exact names. It checks bindings, D1 marker tables, instance identity and public discovery, never a naming prefix. No DB binding or no instance marker table is excluded; incomplete marked schemas, permission failures, timeouts or ambiguous DB bindings remain `unresolved`. Only verified candidates and unresolved Worker names/reason codes are returned, not unrelated configuration or business data. The scan is bounded to 100 Workers and 64 KiB responses; exceeding a bound requires explicit narrowing, never truncated uniqueness. For `selection_required`, show names, origins and instance IDs and ask the user to choose. `incomplete` must not auto-select the remaining candidate; `single_candidate` may propose a target in the authorization plan only. Skip discovery when the target is already exact.

1. Verify that the installed, trusted Skill release lists `owner-recovery discover`, `owner-recovery inspect`, `plan owner-recovery`, and `owner-recovery execute` in `help`. This repository change does not update the current plugin; install/update a release containing the commands only with the user's authorization.
2. Run `owner-recovery inspect` with `instanceId`, `accountId`, `workerName`, `d1Name`, `databaseId`, `apiOrigin`, `wranglerExecutable`, and exactly one of `cloudflareProfile` or `contextDirectory`. It accepts no `taskId`. It checks the exact Cloudflare resources and Worker DB binding, D1 Instance/original Owner/preferred origin, and public discovery/health. Cloudflare authority and all identity facts must agree; names alone are insufficient.
3. Run `plan owner-recovery` with those same fields plus `taskId`. It repeats the read-only preflight and freezes the exact target and recovery effects. Show the plan/digest, same Owner, all-old-API-Credential revocation, Passkey preservation, and private replacement storage before requesting authorization. The plan creates no replacement secret and performs no remote mutation.
4. Use `journal create` with `instanceId`, `plan.operation_id` as `operationId`, and `plan`; then `journal authorize` with `instanceId`, `operationId`, `taskId`, and the returned `plan_digest` as `planDigest` after authorization. Do not substitute a deployment/bootstrap journal or grant a different operation the approval.
5. Run `owner-recovery execute` with `instanceId`, `operationId`, `taskId`, and `plan`. It prepares the replacement in private pending storage, performs a parameterized Cloudflare D1 REST query batch, and reads back the exact operation commit, `owner.credential_recovered` audit, and Credential before authenticated identity verification and local promotion. Cloudflare authentication stays inside the helper. Neither plaintext Credential nor authentication headers enter command arguments, ordinary output, logs, or journal. This does not deploy a Worker, migrate schema, create a new Owner, or call an application recovery endpoint.
6. After interruption or an uncertain response, rerun the same authorized execution with the same plan, journal, and pending/current secret. Readback determines whether to finalize an existing commit or resume untouched state; partial state or drift stops. Never generate another secret or replay first-deployment bootstrap commands. If a hard-killed process leaves `owner-recovery.lock`, verify its recorded PID is no longer running before removing only that exact lock. Retain pending/current and journal files.
...
4. Create `plan instance-upgrade` with the exact resources, current binding readback, unchanged Owner Principal/Credential fingerprint, target compatibility, ordered migration delta, restore evidence, and the public upgrade migration execution constraints `mode: single_query` and `max_sql_bytes: 24576`. These constraints are part of the plan digest; an older plan cannot silently switch execution paths. Authorize its task/operation/digest and create the frozen config from the installed Service cache.
5. Run the fixed migration readback. For each planned migration: apply only the next pending verified canonical bundle migration by passing its complete, unchanged public SQL in one `wrangler d1 execute --remote --command=<SQL>` invocation to `/query`. Limit each SQL to 24 KiB UTF-8 (24,576 bytes), a conservative Windows argv compatibility bound rather than the D1 platform maximum. Stop before writes if oversized; never split statements/chunks or fall back to `--file`, including after failure. This rule does not change checksum or Owner-bootstrap file execution. Read back again; require `migrations assess-ledger-recovery` to prove the one expected post-apply missing checksum; write the plan-bound fixed-path SQL; record it; then read back and reconcile again. Do not skip the post-apply proof.
...
Default first deployment remains one Worker and one D1. Enable attachments through an explicit Instance upgrade (`plan instance-upgrade`, `attachments: {"bucket_name":"<exact-name>","create":true}`) with a compatible release (schema 4 or later). Freeze one private Standard R2 bucket, `ATTACHMENTS`, the hourly `17 * * * *` cleanup trigger, subscription and billable usage, and the release-specific application capacity policy. Deployment never chooses a byte limit or changes an existing Owner setting; an application budget is not a Cloudflare billing cap. This does not authorize subscription changes.

First read the exact bucket/account with `runtime r2-storage-readback`. New names must be absent; an unknown bucket cannot be adopted, even if it has a matching-looking marker. After plan authorization, `deployment provision-r2-storage` journals creation, writes and verifies the Instance marker, and rejects public access. For an existing bucket, omit `attachments` to preserve it and supply its private `currentReceiptPath` when verifying through the provision command. A missing or invalid marker stops; it is never repaired by assuming ownership.

Worker deployment checks current deployment/bindings and Cron schedules before writing, then verifies the actual deployed R2 binding, schedule and bucket before finalization. Unexpected Cron triggers stop before Wrangler can overwrite them. The new receipt preserves the bucket. Resume only the same authorized operation after readback; uncertain creation without a successful journal record stops. Buckets are never automatically replaced, removed, emptied or deleted.

Attachment capacity is an application setting, not a deployment parameter. After enabling R2, direct the Owner to management settings to explicitly choose a positive byte limit or unlimited capacity. Until configured, only new upload reservations are paused. The migration from the former fixed 1 GiB policy also requires an explicit Owner choice; it does not silently select unlimited capacity. Deployment must not write the D1 setting or overwrite a previous Owner choice. This Owner-configured policy applies to schema 7 or later. Older schema 4–6 release plans retain their historical fixed 1 GiB policy and cannot claim the newer setting exists.

## Optional Cloudflare usage configuration

Usage statistics are enabled by default and refresh on demand, never on a statistics Cron. Default deployments require no analytics token and create no resources: without complete configuration the API reports `not_configured`, while the attachment application budget is available. Only the existing attachment cleanup retains its hourly trigger.

`plan instance-upgrade` accepts non-secret `usageAnalytics: { enabled: true, account_id, d1_database_id, r2_bucket_name: null }`. Resource identifiers must match this deployment; omitted account/database are resolved from its frozen target. Use `enabled: false` to explicitly disable cloud analytics. Omission preserves existing enabled or disabled configuration and any existing `USAGE_ANALYTICS_TOKEN` secret binding. Missing secret is permitted and means not configured, not zero usage. Plan changes still require deployment authorization; the same Service artifact is permitted when only analytics configuration changes.

Generated vars are `USAGE_ANALYTICS_ENABLED`, `USAGE_ACCOUNT_ID`, `USAGE_D1_DATABASE_ID`, and optionally `USAGE_R2_BUCKET_NAME`. Never put a token value in input, plans, CLI arguments, journals, receipts, Issues or ordinary output. There is no secret-write command. Provision an independently authorized read-only Worker Secret separately through an approved secure Cloudflare input; never reuse/upload local Wrangler OAuth. Independently validate exact account and minimum Analytics permissions. Preflight verifies unchanged Worker identity and redacted bindings; postdeploy readback proves planned vars and preservation of any secret binding. Binding presence alone does not prove Analytics access. Explicitly refresh through the Owner API after authorized deployment and record permission verification that remains unresolved.
