...
- [`0.1.0-alpha.14` GitHub 测试发行版](https://github.com/breakstring/cfKanban/releases/tag/0.1.0-alpha.14) 提供不可变的 Skill 与 Service bundle；它保留 alpha.13 的部署恢复安全，并让创建看板时可以直接使用用户选择的 key 大小写：管理 Skill 会展示并提交小写 Workspace key 和大写 Project key，不再要求用户重新输入。
- 机器可读的测试入口是 [`prerelease.json`](https://github.com/breakstring/cfKanban/releases/download/0.1.0-alpha.14/prerelease.json)。
...
当前测试预览阶段还没有稳定部署目标。Skill 应该直接说明这一点，并可以把 `0.1.0-alpha.14` 作为需要你明确选择的测试版本；它不能静默选择测试版、marketplace cache 或当前工作目录。
