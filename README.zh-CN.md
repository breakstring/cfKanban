...
- [`0.1.0-alpha.27` GitHub 测试发行版](https://github.com/breakstring/cfKanban/releases/tag/0.1.0-alpha.27) 提供不可变的 Skill 与 Service bundle；部署 Skill 现在会明确说明 named profile 场景仍需私有 Wrangler 上下文，并在缺少该输入时返回专用恢复合同。
- 机器可读的测试入口是 [`prerelease.json`](https://github.com/breakstring/cfKanban/releases/download/0.1.0-alpha.27/prerelease.json)。
...
当前测试预览阶段还没有稳定部署目标。Skill 应该直接说明这一点，并可以把 `0.1.0-alpha.27` 作为需要你明确选择的测试版本；它不能静默选择测试版、marketplace cache 或当前工作目录。
