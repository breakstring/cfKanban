...
- [`1.0.0-rc.3` GitHub 测试发行版](https://github.com/breakstring/cfKanban/releases/tag/1.0.0-rc.3) 提供用于跨系统候选版验收的不可变 Skill 与 Service bundle，新增实例内唯一用户名称、按名称精确查询可分配对象、可选完成说明，以及 Owner 凭据全失恢复。
- 升级后 Owner 需明确设置附件容量才能新增上传，已有文件保持可访问；Cloudflare 统计需单独配置只读 Token。
- 机器可读的测试入口是 [`prerelease.json`](https://github.com/breakstring/cfKanban/releases/download/1.0.0-rc.3/prerelease.json)。
...
`1.0.0-rc.3` 包含四个 Skills：一个入门指南和三个操作技能：
...
当前测试预览阶段还没有稳定部署目标。Skill 应该直接说明这一点，并可以把 `1.0.0-rc.3` 作为需要你明确选择的测试版本；它不能静默选择测试版、marketplace cache 或当前工作目录。
