...
- [`0.1.0-alpha.6` GitHub 测试发行版](https://github.com/breakstring/cfKanban/releases/tag/0.1.0-alpha.6) 提供了不可变的 Skill 与 Service bundle，支持 Node.js 26、canonical Skill 更新、既有 Tool Runtime 复用检测、确定性的 Cloudflare 登录计划，以及经过验证的 D1 创建与读回，可用于测试。
- 机器可读的测试入口是 [`prerelease.json`](https://github.com/breakstring/cfKanban/releases/download/0.1.0-alpha.6/prerelease.json)。
...
当前测试预览阶段还没有稳定部署目标。Skill 应该直接说明这一点，并可以把 `0.1.0-alpha.6` 作为需要你明确选择的测试版本；它不能静默选择测试版、marketplace cache 或当前工作目录。
