...
- [`0.1.0-alpha.25` GitHub 测试发行版](https://github.com/breakstring/cfKanban/releases/tag/0.1.0-alpha.25) 提供不可变的 Skill 与 Service bundle；Web 与 Skills 采用同一套可验证的错误恢复矩阵，诊断事实随时可查但不抢用户操作的注意力，同时更新公开的 Agent-first 产品表达。
- 机器可读的测试入口是 [`prerelease.json`](https://github.com/breakstring/cfKanban/releases/download/0.1.0-alpha.25/prerelease.json)。
...
当前测试预览阶段还没有稳定部署目标。Skill 应该直接说明这一点，并可以把 `0.1.0-alpha.25` 作为需要你明确选择的测试版本；它不能静默选择测试版、marketplace cache 或当前工作目录。
