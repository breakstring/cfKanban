...
- [`0.1.0-alpha.11` GitHub 测试发行版](https://github.com/breakstring/cfKanban/releases/tag/0.1.0-alpha.11) 提供不可变的 Skill 与 Service bundle；它保留简化后的 Wrangler profile/account 处理，并补齐同 plan 绑定的 Owner bootstrap、准确身份读回和脱敏 deployment receipt。
- 机器可读的测试入口是 [`prerelease.json`](https://github.com/breakstring/cfKanban/releases/download/0.1.0-alpha.11/prerelease.json)。
...
当前测试预览阶段还没有稳定部署目标。Skill 应该直接说明这一点，并可以把 `0.1.0-alpha.11` 作为需要你明确选择的测试版本；它不能静默选择测试版、marketplace cache 或当前工作目录。
