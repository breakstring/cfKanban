...
- [`0.1.0-alpha.12` GitHub 测试发行版](https://github.com/breakstring/cfKanban/releases/tag/0.1.0-alpha.12) 提供不可变的 Skill 与 Service bundle；它增加同 plan 绑定的零状态读回，只有 D1 证明 Owner bootstrap 完全没有写入时，才允许在中断后安全重试同一 SQL。
- 机器可读的测试入口是 [`prerelease.json`](https://github.com/breakstring/cfKanban/releases/download/0.1.0-alpha.12/prerelease.json)。
...
当前测试预览阶段还没有稳定部署目标。Skill 应该直接说明这一点，并可以把 `0.1.0-alpha.12` 作为需要你明确选择的测试版本；它不能静默选择测试版、marketplace cache 或当前工作目录。
