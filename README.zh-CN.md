...
- [`0.1.0-alpha.33` GitHub 测试发行版](https://github.com/breakstring/cfKanban/releases/tag/0.1.0-alpha.33) 提供不可变的 Skill 与 Service bundle；它新增专用部署/加入指南和公开首页页脚，明确 Public Join CAS 版本，并兼容一次性交付的不透明事件游标。
- 机器可读的测试入口是 [`prerelease.json`](https://github.com/breakstring/cfKanban/releases/download/0.1.0-alpha.33/prerelease.json)。
...
当前测试预览阶段还没有稳定部署目标。Skill 应该直接说明这一点，并可以把 `0.1.0-alpha.33` 作为需要你明确选择的测试版本；它不能静默选择测试版、marketplace cache 或当前工作目录。
