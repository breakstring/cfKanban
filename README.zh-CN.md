...
- [`0.1.0-alpha.30` GitHub 测试发行版](https://github.com/breakstring/cfKanban/releases/tag/0.1.0-alpha.30) 提供不可变的 Skill 与 Service bundle；Owner 侧 CAS 恢复现在也覆盖被其他页面撤销的 Grant 与被关闭的 Public Join，并在写入期间明确锁定相关表单。
- 机器可读的测试入口是 [`prerelease.json`](https://github.com/breakstring/cfKanban/releases/download/0.1.0-alpha.30/prerelease.json)。
...
当前测试预览阶段还没有稳定部署目标。Skill 应该直接说明这一点，并可以把 `0.1.0-alpha.30` 作为需要你明确选择的测试版本；它不能静默选择测试版、marketplace cache 或当前工作目录。
