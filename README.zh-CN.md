...
- [`0.1.0-alpha.21` GitHub 测试发行版](https://github.com/breakstring/cfKanban/releases/tag/0.1.0-alpha.21) 提供不可变的 Skill 与 Service bundle；它收紧 SPA 缓存与 Referrer 响应头，并补齐 Browser Session、剪贴板降级、语言与 Project 状态显示名体验。
- 机器可读的测试入口是 [`prerelease.json`](https://github.com/breakstring/cfKanban/releases/download/0.1.0-alpha.21/prerelease.json)。
...
当前测试预览阶段还没有稳定部署目标。Skill 应该直接说明这一点，并可以把 `0.1.0-alpha.21` 作为需要你明确选择的测试版本；它不能静默选择测试版、marketplace cache 或当前工作目录。
