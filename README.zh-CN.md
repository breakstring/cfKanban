...
- [`0.1.0-alpha.24` GitHub 测试发行版](https://github.com/breakstring/cfKanban/releases/tag/0.1.0-alpha.24) 提供不可变的 Skill 与 Service bundle；切换界面语言时，已经显示的错误和恢复提示会立即随之切换，同时保留请求 ID、稳定资源事实和未提交草稿。
- 机器可读的测试入口是 [`prerelease.json`](https://github.com/breakstring/cfKanban/releases/download/0.1.0-alpha.24/prerelease.json)。
...
当前测试预览阶段还没有稳定部署目标。Skill 应该直接说明这一点，并可以把 `0.1.0-alpha.24` 作为需要你明确选择的测试版本；它不能静默选择测试版、marketplace cache 或当前工作目录。
