...
- [`0.1.0-alpha.1` GitHub 测试发行版](https://github.com/breakstring/cfKanban/releases/tag/0.1.0-alpha.1)提供了不可变的 Skill 与 Service bundle，可用于测试。
- 机器可读的测试入口是 [`prerelease.json`](https://github.com/breakstring/cfKanban/releases/download/0.1.0-alpha.1/prerelease.json)。
...
当前测试预览阶段还没有稳定部署目标。Skill 应该直接说明这一点，并可以把 `0.1.0-alpha.1` 作为需要你明确选择的测试版本；它不能静默选择测试版、marketplace cache 或当前工作目录。
...
当前 Skill 还没有能够冻结上述全部事实的源码专用远端部署计划，因此正确的源码评估会在 Cloudflare 写入前停止。发布并验收首个 immutable release，是启用下方受支持部署流程前仍需完成的前置条件。
...
3. 确认准确的 Cloudflare 账户，只询问 Owner 显示名称等真正缺少的选择；
