...
- [`0.1.0-alpha.29` GitHub 测试发行版](https://github.com/breakstring/cfKanban/releases/tag/0.1.0-alpha.29) 提供不可变的 Skill 与 Service bundle；并发删除／恢复发生状态冲突时，现在会保留本地动作草稿、读回远端事实并展示资源当前版本。
- 机器可读的测试入口是 [`prerelease.json`](https://github.com/breakstring/cfKanban/releases/download/0.1.0-alpha.29/prerelease.json)。
...
当前测试预览阶段还没有稳定部署目标。Skill 应该直接说明这一点，并可以把 `0.1.0-alpha.29` 作为需要你明确选择的测试版本；它不能静默选择测试版、marketplace cache 或当前工作目录。
