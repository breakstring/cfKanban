...
- [`0.1.0-alpha.35` GitHub 测试发行版](https://github.com/breakstring/cfKanban/releases/tag/0.1.0-alpha.35) 提供不可变的 Skill 与 Service bundle；它为 favicon 和 Web 品牌位加入原创的“任务卡自己冲出看板”标志，同时保留严格 CSP 与 Cloudflare 边缘防改写。
- 机器可读的测试入口是 [`prerelease.json`](https://github.com/breakstring/cfKanban/releases/download/0.1.0-alpha.35/prerelease.json)。
...
当前测试预览阶段还没有稳定部署目标。Skill 应该直接说明这一点，并可以把 `0.1.0-alpha.35` 作为需要你明确选择的测试版本；它不能静默选择测试版、marketplace cache 或当前工作目录。
