...
- The [`0.1.0-alpha.14` GitHub prerelease](https://github.com/breakstring/cfKanban/releases/tag/0.1.0-alpha.14) packages immutable Skill and Service bundles for testing. It keeps alpha.13's deployment-recovery safety and makes board creation accept user-chosen key casing: the admin Skill shows and submits the lowercase Workspace key and uppercase Project key without asking the user to retype either value.
- Its machine-readable testing entry is [`prerelease.json`](https://github.com/breakstring/cfKanban/releases/download/0.1.0-alpha.14/prerelease.json).
...
At the current testing-preview stage, no stable deployment target is published. The Skill should say that clearly and may offer the `0.1.0-alpha.14` prerelease as an explicit testing choice; it must never select a prerelease, a marketplace cache, or the current working tree silently.
