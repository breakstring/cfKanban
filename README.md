...
- The [`0.1.0-alpha.17` GitHub prerelease](https://github.com/breakstring/cfKanban/releases/tag/0.1.0-alpha.17) packages immutable Skill and Service bundles for testing. It keeps alpha.16's Service-assigned Credential ID recovery and gives new and existing Principal Invite/Public Join flows one stable secret-safe result envelope.
- Its machine-readable testing entry is [`prerelease.json`](https://github.com/breakstring/cfKanban/releases/download/0.1.0-alpha.17/prerelease.json).
...
At the current testing-preview stage, no stable deployment target is published. The Skill should say that clearly and may offer the `0.1.0-alpha.17` prerelease as an explicit testing choice; it must never select a prerelease, a marketplace cache, or the current working tree silently.
