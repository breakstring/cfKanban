...
- The [`1.0.0-rc.3` GitHub prerelease](https://github.com/breakstring/cfKanban/releases/tag/1.0.0-rc.3) packages immutable Skill and Service bundles for cross-system release-candidate testing, including unique participant names, exact assignee lookup, optional completion summaries, and Owner credential recovery.
...
- Its machine-readable testing entry is [`prerelease.json`](https://github.com/breakstring/cfKanban/releases/download/1.0.0-rc.3/prerelease.json).
...
`1.0.0-rc.3` contains four Skills: an onboarding guide and three operational Skills:
...
At the current testing-preview stage, no stable deployment target is published. The Skill should say that clearly and may offer the `1.0.0-rc.3` prerelease as an explicit testing choice; it must never select a prerelease, a marketplace cache, or the current working tree silently.
