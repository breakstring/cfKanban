...
- The [`0.1.0-alpha.25` GitHub prerelease](https://github.com/breakstring/cfKanban/releases/tag/0.1.0-alpha.25) packages immutable Skill and Service bundles for testing. It gives Web and Skills one verified error/recovery matrix, keeps diagnostics available without crowding the human action, and refreshes the public Agent-first promise.
- Its machine-readable testing entry is [`prerelease.json`](https://github.com/breakstring/cfKanban/releases/download/0.1.0-alpha.25/prerelease.json).
...
At the current testing-preview stage, no stable deployment target is published. The Skill should say that clearly and may offer the `0.1.0-alpha.25` prerelease as an explicit testing choice; it must never select a prerelease, a marketplace cache, or the current working tree silently.
