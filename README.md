...
> Current status: the Foundation contract is Frozen at revision 13 and the Agent Skills contract at revision 14. The minimal Web UI and API/D1 Schema contracts are still Drafts. The repository contains documentation only; there is no application code or authorized implementation plan yet.
...
- Every authenticated Principal can view their stable ID and current display name, and update only their own non-empty display name through the `cfkanban` Skill or the Web “My profile” surface. v0 does not add avatars, email addresses, biographies, or a general user-profile system.
...
The Foundation SPEC is Frozen at revision 13 and the Agent Skills & Bootstrap SPEC at revision 14. The latter now fixes the single missing Owner-name prompt, combined first-join confirmation, and pending-to-current Credential recovery lifecycle. The minimal Web direction, Browser Launch/Session boundaries, Owner Credential anti-lockout, Owner admin Session scope, Passkey direct login, single-Project Public Join, simple re-entry, three mandatory active quotas, defaulted rate-limit deployment configuration, and layered error normalization also remain frozen. Freezing does not authorize implementation. The bilingual Web and self-profile surfaces, CSRF details, exact HTTP/OpenAPI fields, D1 DDL, indexes, and atomic write recipes remain Draft. Long-term physical retention of soft-deleted data remains deferred; v0 exposes no hard-delete API and no complete D1 export, import, or full-database disaster-recovery capability.
