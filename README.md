...
- The [`0.1.0-alpha.26` GitHub prerelease](https://github.com/breakstring/cfKanban/releases/tag/0.1.0-alpha.26) packages immutable Skill and Service bundles for testing. It makes Owner audit scope explicit across the API, Web UI, and Admin Skill, with Project/stream filters and cursor-safe pagination.
- Its machine-readable testing entry is [`prerelease.json`](https://github.com/breakstring/cfKanban/releases/download/0.1.0-alpha.26/prerelease.json).
...
At the current testing-preview stage, no stable deployment target is published. The Skill should say that clearly and may offer the `0.1.0-alpha.26` prerelease as an explicit testing choice; it must never select a prerelease, a marketplace cache, or the current working tree silently.
