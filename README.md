...
- The [`0.1.0-alpha.37` GitHub prerelease](https://github.com/breakstring/cfKanban/releases/tag/0.1.0-alpha.37) packages immutable Skill and Service bundles for testing. It keeps Workspace and Project key validation valid in modern browsers that compile HTML `pattern` attributes with Unicode Sets mode.
- Its machine-readable testing entry is [`prerelease.json`](https://github.com/breakstring/cfKanban/releases/download/0.1.0-alpha.37/prerelease.json).
...
At the current testing-preview stage, no stable deployment target is published. The Skill should say that clearly and may offer the `0.1.0-alpha.37` prerelease as an explicit testing choice; it must never select a prerelease, a marketplace cache, or the current working tree silently.
