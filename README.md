...
- The [`0.1.0-alpha.59` GitHub prerelease](https://github.com/breakstring/cfKanban/releases/tag/0.1.0-alpha.59) packages immutable Skill and Service bundles with Owner usage statistics, shared 15-minute on-demand analytics caching, and Owner-selected attachment capacity limits or unlimited storage.
...
- Its machine-readable testing entry is [`prerelease.json`](https://github.com/breakstring/cfKanban/releases/download/0.1.0-alpha.59/prerelease.json).
...
```sh
codex plugin marketplace add https://github.com/breakstring/cfKanban.git --ref 0.1.0-alpha.59
...
At the current testing-preview stage, no stable deployment target is published. The Skill should say that clearly and may offer the `0.1.0-alpha.59` prerelease as an explicit testing choice; it must never select a prerelease, a marketplace cache, or the current working tree silently.
