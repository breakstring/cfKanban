...
- The [`0.1.0-alpha.6` GitHub prerelease](https://github.com/breakstring/cfKanban/releases/tag/0.1.0-alpha.6) packages immutable Skill and Service bundles for testing, including Node.js 26 support, canonical Skill updates, reusable Tool Runtime detection, deterministic Cloudflare login planning, and verified D1 creation/readback.
- Its machine-readable testing entry is [`prerelease.json`](https://github.com/breakstring/cfKanban/releases/download/0.1.0-alpha.6/prerelease.json).
...
At the current testing-preview stage, no stable deployment target is published. The Skill should say that clearly and may offer the `0.1.0-alpha.6` prerelease as an explicit testing choice; it must never select a prerelease, a marketplace cache, or the current working tree silently.
