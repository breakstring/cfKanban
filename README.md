...
- The [`0.1.0-alpha.30` GitHub prerelease](https://github.com/breakstring/cfKanban/releases/tag/0.1.0-alpha.30) packages immutable Skill and Service bundles for testing. Owner-side CAS recovery now also handles remotely revoked Grants and disabled Public Join policies, while affected forms visibly lock during an in-flight write.
- Its machine-readable testing entry is [`prerelease.json`](https://github.com/breakstring/cfKanban/releases/download/0.1.0-alpha.30/prerelease.json).
...
At the current testing-preview stage, no stable deployment target is published. The Skill should say that clearly and may offer the `0.1.0-alpha.30` prerelease as an explicit testing choice; it must never select a prerelease, a marketplace cache, or the current working tree silently.
