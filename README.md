...
- The [`0.1.0-alpha.45` GitHub prerelease](https://github.com/breakstring/cfKanban/releases/tag/0.1.0-alpha.45) packages immutable Skill and Service bundles for testing. Container recovery now previews the public summary, roles, and named active usage and limits before restoring Public Join.
- Its machine-readable testing entry is [`prerelease.json`](https://github.com/breakstring/cfKanban/releases/download/0.1.0-alpha.45/prerelease.json).
...
At the current testing-preview stage, no stable deployment target is published. The Skill should say that clearly and may offer the `0.1.0-alpha.45` prerelease as an explicit testing choice; it must never select a prerelease, a marketplace cache, or the current working tree silently.
