...
- The [`0.1.0-alpha.22` GitHub prerelease](https://github.com/breakstring/cfKanban/releases/tag/0.1.0-alpha.22) packages immutable Skill and Service bundles for testing. It adds complete cursor pagination, shared CAS recovery with retained drafts, write re-entry fences, and structured completion history across the Web UI.
- Its machine-readable testing entry is [`prerelease.json`](https://github.com/breakstring/cfKanban/releases/download/0.1.0-alpha.22/prerelease.json).
...
At the current testing-preview stage, no stable deployment target is published. The Skill should say that clearly and may offer the `0.1.0-alpha.22` prerelease as an explicit testing choice; it must never select a prerelease, a marketplace cache, or the current working tree silently.
