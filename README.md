...
- The [`0.1.0-alpha.53` GitHub prerelease](https://github.com/breakstring/cfKanban/releases/tag/0.1.0-alpha.53) packages immutable Skill and Service bundles for testing. Deployment, joining, and installation guides now address the receiving Agent directly, with explicit command, authorization, and verification checkpoints.
- Its machine-readable testing entry is [`prerelease.json`](https://github.com/breakstring/cfKanban/releases/download/0.1.0-alpha.53/prerelease.json).
...
```sh
codex plugin marketplace add https://github.com/breakstring/cfKanban.git --ref 0.1.0-alpha.53
...
At the current testing-preview stage, no stable deployment target is published. The Skill should say that clearly and may offer the `0.1.0-alpha.53` prerelease as an explicit testing choice; it must never select a prerelease, a marketplace cache, or the current working tree silently.
