...
- The [`0.1.0-alpha.15` GitHub prerelease](https://github.com/breakstring/cfKanban/releases/tag/0.1.0-alpha.15) packages immutable Skill and Service bundles for testing. It keeps alpha.14's key-casing normalization and fixes same-publisher Skill updates from receipts written by earlier releases, while still stopping on an actual publisher or artifact-origin change.
- Its machine-readable testing entry is [`prerelease.json`](https://github.com/breakstring/cfKanban/releases/download/0.1.0-alpha.15/prerelease.json).
...
At the current testing-preview stage, no stable deployment target is published. The Skill should say that clearly and may offer the `0.1.0-alpha.15` prerelease as an explicit testing choice; it must never select a prerelease, a marketplace cache, or the current working tree silently.
