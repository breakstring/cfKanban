...
- The [`0.1.0-alpha.43` GitHub prerelease](https://github.com/breakstring/cfKanban/releases/tag/0.1.0-alpha.43) packages immutable Skill and Service bundles for testing. Its source adds draft-first, resumable release publication with exact asset verification; the Service retains the UTF-8 guide fix from alpha.42.
- Its machine-readable testing entry is [`prerelease.json`](https://github.com/breakstring/cfKanban/releases/download/0.1.0-alpha.43/prerelease.json).
...
At the current testing-preview stage, no stable deployment target is published. The Skill should say that clearly and may offer the `0.1.0-alpha.43` prerelease as an explicit testing choice; it must never select a prerelease, a marketplace cache, or the current working tree silently.
