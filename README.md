...
- The canonical source is a monorepo, while the v0 cloud topology remains one Worker plus one D1. Prebuilt Web assets ship in the same Service deployment bundle through Workers Static Assets; v0 creates neither a Pages project nor a KV namespace.
...
- Each instance publishes one Owner-selected preferred API origin through a dynamic, public, non-secret discovery document. An existing agent may automatically rebind only when its current trusted origin announces a higher version and a credential-free probe proves the new HTTPS origin is the same instance; unfamiliar origins still require explicit trust. Domain bindings remain external control-plane configuration. Web sessions remain origin-specific, while v0 deliberately does not share Passkeys across hostnames.
