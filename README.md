...
- The [`0.1.0-alpha.27` GitHub prerelease](https://github.com/breakstring/cfKanban/releases/tag/0.1.0-alpha.27) packages immutable Skill and Service bundles for testing. Its deployment Skill now makes the required private Wrangler context explicit even with a named profile and returns a dedicated recovery contract when that input is missing.
- Its machine-readable testing entry is [`prerelease.json`](https://github.com/breakstring/cfKanban/releases/download/0.1.0-alpha.27/prerelease.json).
...
At the current testing-preview stage, no stable deployment target is published. The Skill should say that clearly and may offer the `0.1.0-alpha.27` prerelease as an explicit testing choice; it must never select a prerelease, a marketplace cache, or the current working tree silently.
