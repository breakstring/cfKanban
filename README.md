...
- The [`0.1.0-alpha.42` GitHub prerelease](https://github.com/breakstring/cfKanban/releases/tag/0.1.0-alpha.42) packages immutable Skill and Service bundles for testing. It fixes Chinese decoding in the public Markdown guides by explicitly serving UTF-8 plain text.
- Its machine-readable testing entry is [`prerelease.json`](https://github.com/breakstring/cfKanban/releases/download/0.1.0-alpha.42/prerelease.json).
...
At the current testing-preview stage, no stable deployment target is published. The Skill should say that clearly and may offer the `0.1.0-alpha.42` prerelease as an explicit testing choice; it must never select a prerelease, a marketplace cache, or the current working tree silently.
