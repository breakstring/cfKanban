...
- The [`0.1.0-alpha.12` GitHub prerelease](https://github.com/breakstring/cfKanban/releases/tag/0.1.0-alpha.12) packages immutable Skill and Service bundles for testing. It adds a plan-bound zero-state readback so an interrupted Owner bootstrap can safely retry the same SQL only when D1 proves that nothing was written.
- Its machine-readable testing entry is [`prerelease.json`](https://github.com/breakstring/cfKanban/releases/download/0.1.0-alpha.12/prerelease.json).
...
At the current testing-preview stage, no stable deployment target is published. The Skill should say that clearly and may offer the `0.1.0-alpha.12` prerelease as an explicit testing choice; it must never select a prerelease, a marketplace cache, or the current working tree silently.
