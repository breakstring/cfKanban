...
- The [`0.1.0-alpha.13` GitHub prerelease](https://github.com/breakstring/cfKanban/releases/tag/0.1.0-alpha.13) packages immutable Skill and Service bundles for testing. It keeps the zero-state Owner-bootstrap recovery from alpha.12 and makes that safe retry part of the full deployment authorization, so users are not asked to repeat a technical confirmation when nothing has drifted.
- Its machine-readable testing entry is [`prerelease.json`](https://github.com/breakstring/cfKanban/releases/download/0.1.0-alpha.13/prerelease.json).
...
At the current testing-preview stage, no stable deployment target is published. The Skill should say that clearly and may offer the `0.1.0-alpha.13` prerelease as an explicit testing choice; it must never select a prerelease, a marketplace cache, or the current working tree silently.
