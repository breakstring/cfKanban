...
- The [`0.1.0-alpha.35` GitHub prerelease](https://github.com/breakstring/cfKanban/releases/tag/0.1.0-alpha.35) packages immutable Skill and Service bundles for testing. It gives cfKanban an original self-moving-card mark across the favicon and Web chrome while retaining the strict CSP and Cloudflare edge-transform protection.
- Its machine-readable testing entry is [`prerelease.json`](https://github.com/breakstring/cfKanban/releases/download/0.1.0-alpha.35/prerelease.json).
...
At the current testing-preview stage, no stable deployment target is published. The Skill should say that clearly and may offer the `0.1.0-alpha.35` prerelease as an explicit testing choice; it must never select a prerelease, a marketplace cache, or the current working tree silently.
