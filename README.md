...
- The [`1.0.0-rc.6` GitHub prerelease](https://github.com/breakstring/cfKanban/releases/tag/1.0.0-rc.6) packages immutable Skill and Service bundles for cross-system release-candidate testing, reducing D1 query costs with bounded pagination, targeted authorization queries, and schema 10 indexes.
...
- Its machine-readable testing entry is [`prerelease.json`](https://github.com/breakstring/cfKanban/releases/download/1.0.0-rc.6/prerelease.json).
...
`1.0.0-rc.6` contains four Skills: an onboarding guide and three operational Skills:
...
At the current testing-preview stage, no stable deployment target is published. The Skill should say that clearly and may offer the `1.0.0-rc.6` prerelease as an explicit testing choice; it must never select a prerelease, a marketplace cache, or the current working tree silently.
