...
- The [`0.1.0-alpha.24` GitHub prerelease](https://github.com/breakstring/cfKanban/releases/tag/0.1.0-alpha.24) packages immutable Skill and Service bundles for testing. It makes already-visible errors and recovery guidance react immediately to locale changes while preserving request IDs, stable resource facts, and unsaved drafts.
- Its machine-readable testing entry is [`prerelease.json`](https://github.com/breakstring/cfKanban/releases/download/0.1.0-alpha.24/prerelease.json).
...
At the current testing-preview stage, no stable deployment target is published. The Skill should say that clearly and may offer the `0.1.0-alpha.24` prerelease as an explicit testing choice; it must never select a prerelease, a marketplace cache, or the current working tree silently.
