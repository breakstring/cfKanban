...
- The [`0.1.0-alpha.20` GitHub prerelease](https://github.com/breakstring/cfKanban/releases/tag/0.1.0-alpha.20) packages immutable Skill and Service bundles for testing. It fixes existing-Instance Worker readback to use Wrangler's authoritative current-production status instead of selecting from deployment history.
- Its machine-readable testing entry is [`prerelease.json`](https://github.com/breakstring/cfKanban/releases/download/0.1.0-alpha.20/prerelease.json).
...
At the current testing-preview stage, no stable deployment target is published. The Skill should say that clearly and may offer the `0.1.0-alpha.20` prerelease as an explicit testing choice; it must never select a prerelease, a marketplace cache, or the current working tree silently.
